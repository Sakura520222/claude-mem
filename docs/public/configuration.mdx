...
| `CLAUDE_MEM_PROVIDER`         | `claude`                        | AI provider: `claude`, `gemini`, `openrouter`, or `custom` (installer-only alias that points the OpenRouter client at any OpenAI-compatible endpoint) |
...
| `CLAUDE_MEM_OPENROUTER_BASE_URL`             | —                           | Optional: point the OpenRouter client at **any OpenAI-compatible endpoint** (e.g. `https://api.deepseek.com`, `http://localhost:1234/v1` for LM Studio). `/chat/completions` is appended automatically. See [Custom Provider](usage/custom-provider). |
...
See [OpenRouter Provider](usage/openrouter-provider) for detailed configuration, free model list, and usage guide. For bringing your own OpenAI-compatible endpoint (DeepSeek, OpenAI, LM Studio, Ollama, vLLM, LiteLLM), see [Custom Provider](usage/custom-provider).
